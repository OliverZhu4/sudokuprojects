...
For example, the command to clone the repository for Homework 5 (Part I) that is assigned to a student with GitHub username `vandercc` in the Spring 2021 edition of CS 3270 would look like this:

```
git clone https://github.com/vu-cs3270-s21/hw05-i-vandercc.git
```

If the command is successful, you now have a local copy of your GitHub repository for Homework 5 (Part I).
...
If you want to just add (or stage) one particular file, e.g., you only want to commit `hw05-i.rkt`, in the prompt/terminal, go into (i.e., `cd`) the repository's root folder and then run the following command:
